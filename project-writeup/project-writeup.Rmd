...
fullSet <-read_csv("/cloud/project/data/master.csv")
```
...
```
The countries that do not have HDI calculated for them are Aruba, Puerto Rico, Korea, and Russia. 
...
```{r}
gapminderNew = gapminder %>% select(country, continent, region)

gapminderNew <- data.frame(lapply(gapminderNew, as.character), stringsAsFactors=FALSE)

gapminderNew$country <- replace(gapminderNew$country, gapminderNew$country=="Kyrgyz Republic", "Kyrgyzstan")

gapminderNew$country <- replace(gapminderNew$country, gapminderNew$country=="South Korea", "Republic of Korea")

gapminderNew$country <- replace(gapminderNew$country, gapminderNew$country=="Russia", "Russian Federation")

gapminderNew$country <- replace(gapminderNew$country, gapminderNew$country=="St. Lucia", "Saint Lucia")

gapminderNew <- data.frame(lapply(gapminderNew, as.character), stringsAsFactors=TRUE)

suicide <- merge(suicideNew, gapminderNew, by = "country", all.x = TRUE)
```
