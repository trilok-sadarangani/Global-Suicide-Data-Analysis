...
```{r}
ggplot(data=suicideFinal, mapping=aes(x=`suicides/100k pop`)) + geom_histogram() +
...
Log transforming the data set, we notice that the distribution of GDPs of countries has a bimonial distribution.
...
```{r}
pairs(`suicides/100k pop` ~ log(`gdp_for_year ($)`) + log(`gdp_per_capita ($)`), data = suicideFinal)
```
...
## Section 3. Data
```{r}
model1 <- lm(`suicides/100k pop` ~ country+year+sex+age+suicides_no + population + country-year + HDI + (`gdp_for_year ($)`) + (`gdp_per_capita ($)`) + continent + region + generation, data = suicideFinal)
...
We chose a multiple linear regression model because we want to predict the value of a variable based on multile different independent variables. Based on backward selection, we found that continent, region, generation, HDI, gdp/year and gdp/capita, so decided to include country, sex, age, suicides_no and population.

# Testing of Interesting Interactions
```{r}
model2 <- lm(formula = `suicides/100k pop` ~ country*sex + sex + age + suicides_no + population, data = suicideFinal)

anova(model1,model2)
```
The F statistic is 1.4266 and the p-value is 0.008494, which is  small. We can can conclude that the interaction effect of country and sex is a significant predictor.

```{r}
model3 <- lm(formula = `suicides/100k pop` ~ country*age + sex + age + suicides_no + population, data = suicideFinal)

anova(model1,model3)
```
The F statistic is 2.2847 and the p-value is 2.2e-16, which is  very small. We can can conclude that the interaction effect of country and age is a significant predictor.


```{r}
model4 <- lm(formula = `suicides/100k pop` ~ country*suicides_no + sex + age + suicides_no + population, data = suicideFinal)

anova(model1,model4)
```
The F statistic is 6.7103 and the p-value is 2.2e-16, which is  very small. We can can conclude that the interaction effect of country and suicides_no is a significant predictor.

```{r}
model5 <- lm(formula = `suicides/100k pop` ~ country*population + sex + age + suicides_no + population, data = suicideFinal)

anova(model1,model5)
```
The F statistic is 2.6156 and the p-value is 2.103e-12, which is  very small. We can can conclude that the interaction effect of country and population is a significant predictor.


```{r}
model6 <- lm(formula = `suicides/100k pop` ~ country + sex*age + age + suicides_no + population, data = suicideFinal)

anova(model1,model6)
```
The F statistic is 19.946 and the p-value is 2.2e-16, which is  very small. We can can conclude that the interaction effect of sex and age is a significant predictor.


```{r}
model7 <- lm(formula = `suicides/100k pop` ~ country + sex*suicides_no + age + suicides_no + population, data = suicideFinal)

anova(model1,model7)
```
The F statistic is 8.8535 and the p-value is 0.002999, which is  very small. We can can conclude that the interaction effect of sex and suicides_no is a significant predictor.


```{r}
model8 <- lm(formula = `suicides/100k pop` ~ country + sex*population + age + suicides_no + population, data = suicideFinal)

anova(model1,model8)
```
The F statistic is 2.9748 and the p-value is 0.08489, which is larger than 0.05. We can can conclude that the interaction effect of sex and population is not significant predictor.


```{r}
model9 <- lm(formula = `suicides/100k pop` ~ country + sex + age*suicides_no + suicides_no + population, data = suicideFinal)

anova(model1,model9)
```
The F statistic is 6.2058 and the p-value is 1.132e-05, which is smaller than 0.05. We can can conclude that the interaction effect of age and suicides_no is significant predictor.


```{r}
model10 <- lm(formula = `suicides/100k pop` ~ country + sex + age*population + suicides_no + population, data = suicideFinal)

anova(model1,model10)
```
The F statistic is 3.7101 and the p-value is 0.002483, which is smaller than 0.05. We can can conclude that the interaction effect of age and population is significant predictor.

```{r}
model11 <- lm(formula = `suicides/100k pop` ~ country + sex + age + suicides_no*population + population, data = suicideFinal)

anova(model1,model11)
```
The F statistic is 43.415 and the p-value is 7.291e-11, which is smaller than 0.05. We can can conclude that the interaction effect of suicides_no and population is significant predictor.

So, we can conclude that to not include the interaction effect between sex and population. Our updated model will be: 

```{r}
finalmodel <- lm(formula = `suicides/100k pop` ~ country + sex + age + suicides_no + population + country*sex + country*age + country*suicides_no + country*population + sex*age + sex*suicides_no + age*suicides_no + age*population + suicides_no*population, data = suicideFinal)
