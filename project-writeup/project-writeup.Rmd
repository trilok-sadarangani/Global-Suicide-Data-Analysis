...
anova(model1,model2)
```
The F statistic is 2.4591 and the p-value is 0.003639, which is  small. We can can conclude that the interaction effect of HDI and region is a significant predictor.
...
anova(model1,model3)
```
The F statistic is 4.3923  and the p-value is 4.641e-08, which is very low We can can conclude that the interaction effect of region and population is  a significant predictor.
...
anova(model1,model4)
```
The F statistic is 2.216  and the p-value is 0.007552 , which is below .05. We can can conclude that the interaction effect of region and GDP for year is  a significant predictor.
...
anova(model1,model5)
```
The F statistic is 1.2491   and the p-value is 0.2387, which is above .05 We can can conclude that the interaction effect of region and gdp per capita is not a significant predictor.
...
```
The F statistic is 13.857   and the p-value is 4.027e-13, which is smaller than 0.05. We can can conclude that the interaction effect of age and HDI is significant predictor.
...
```
The F statistic is 3.0279 and the p-value is 0.01015, which is smaller than 0.05. We can can conclude that the interaction effect of age and gdp per capita is a significant predictor.
...
```
The F statistic is 27.466   and the p-value is 2.2e-16, which is smaller than 0.05. We can can conclude that the interaction effect of age and gdp for year is significant predictor.
...
```
The F statistic is 23.793  and the p-value is 2.2e-16, which is smaller than 0.05. We can can conclude that the interaction effect of age and gdp per capita is a significant predictor.

```{r}
```

From this, analysis, we will include 7 interactoin effects in our new model
```{r}
...
```{r}
ggplot(data=suicideFinal,mapping=aes(x=year,y=`suicides/100k pop`,color=as.factor(region))) +
geom_smooth(method="lm", se=FALSE) + labs(title="Relationship between suicides/100k pop and year with regards to region view", 
x="year", 
y="year",
color="region")

ggplot(data=suicideFinal,mapping=aes(x=suicides_no,y=`suicides/100k pop`,color=as.factor(region))) +
geom_smooth(method="lm", se=FALSE) + labs(title="Relationship between suicides/100k pop and suicides_no with regards to region view", 
x="suicides_no", 
y="suicides",
color="region")

ggplot(data=suicideFinal,mapping=aes(x=population,y=`suicides/100k pop`,color=as.factor(region))) +
geom_smooth(method="lm", se=FALSE) + labs(title="Relationship between suicides/100k pop and population with regards to region view", 
x="population", 
y="suicides",
color="region")

ggplot(data=suicideFinal,mapping=aes(x=HDI,y=`suicides/100k pop`,color=as.factor(region))) +
geom_smooth(method="lm", se=FALSE) + labs(title="Relationship between suicides/100k pop and hdi with regards to region view", 
x="hdi", 
y="suicides",
color="region")
...
color="region")
```
There is a significant interaction effect between bedrooms and waterfront because the lines are not relatively parallel, as the slopes of the two lines look to be quite different. The rate of increase of the log price of houses on the waterfront is greater than the rate of increase of the log price of houses not on the waterfront. This means that as the number of bedrooms of houses on the waterfront increases by one, the log price increases as well and increases more than the log price for houses not on the waterfront as the number of bedrooms increases by one.
...
leverage_threshold <- 2*(14+1)/nrow(suicidesoutput)
...
suicidesoutput %>% filter(.hat > leverage_threshold) %>% 
  select(region, population - mean(population), HDI - mean(HDI), GDP - mean(GDP), `gdp_for_year ($)` - mean(`gdp_for_year ($)`), `gdp_per_capita ($)` - mean(`gdp_per_capita ($)`))
```
...
suicidesoutput %>% filter(abs(.std.resid) > 2) %>% 
  select(region, population - mean(population), HDI - mean(HDI), GDP - mean(GDP), `gdp_for_year ($)` - mean(`gdp_for_year ($)`), `gdp_per_capita ($)` - mean(`gdp_per_capita ($)`))
```
There are 7 observations that are considered to have standarized residuals with large magnitude.
...
There are 7 observations flagged as having standardized residuals with large magnitude and 185 observations in total, so approximately 3.78% of observations have standardized residuals with magnitude greater than 2. This is not a concern because this is less than 5%.
