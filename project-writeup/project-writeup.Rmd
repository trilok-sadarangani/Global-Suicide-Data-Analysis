...
However, since this response variable has many negative infinity values (this stems from the fact that some countries have 0 suicides), we will first increase the suicide number by 1 for each country and then recalculate suicides per 100k.

```{r}
suicide <- suicide %>% mutate (`suicides_no` = `suicides_no`+1)
```

```{r}
suicide <- suicide %>% mutate (`suicides/100k pop` = `suicides_no`/(population/100000))
```
