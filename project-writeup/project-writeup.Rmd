...
First, we will mean center our quantitative variables.
```{r}
suicideFinal <- suicideFinal %>%
  mutate(population = population - mean(population), HDI = HDI - mean(HDI), `gdp_for_year ($)` = `gdp_for_year ($)` - mean(`gdp_for_year ($)`), `gdp_per_capita ($)` = `gdp_per_capita ($)` - mean(`gdp_per_capita ($)`))
```
...
```{r}
model2 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*age, data = suicideFinal)

anova(model1,model2)
```
The F statistic is 15.229 and the p-value is 1.862e-14, which is  small. We can can conclude that the interaction effect of age and sex is a significant predictor.

```{r}
model3 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*population, data = suicideFinal)

anova(model1,model3)
```
The F statistic is 2.1534 and the p-value is 0.1426-16, which is high. We can can conclude that the interaction effect of sex and population is not a significant predictor.


```{r}
model4 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*HDI, data = suicideFinal)

anova(model1,model4)
```
The F statistic is .3308 and the p-value is 0.5653, which is  high. We can can conclude that the interaction effect of sex and HDI is not a significant predictor.

```{r}
model5 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*`gdp_for_year ($)`, data = suicideFinal)

anova(model1,model5)
```
The F statistic is 13.261  and the p-value is 0.0002844, which is very small. We can can conclude that the interaction effect of sex and gdp_for_year ($) is a significant predictor.


```{r}
model6 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*`gdp_per_capita ($)`, data = suicideFinal)

anova(model1,model6)
```
The F statistic is .87421 and the p-value is 0.2373, which is high. We can can conclude that the interaction effect of sex and gdp per capita is not a significant predictor.


```{r}
model7 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + sex*region, data = suicideFinal)

anova(model1,model7)
```
The F statistic is 2.675 and the p-value is 0.0005204, which is low We can can conclude that the interaction effect of sex and region is a significant predictor.


```{r}
model8 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + age*population, data = suicideFinal)

anova(model1,model8)
```
The F statistic is 3.2618 and the p-value is 0.006284 , which is small We can can conclude that the interaction effect of age and population is a significant predictor.


```{r}
model9 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + age*HDI, data = suicideFinal)

anova(model1,model9)
```
The F statistic is 12.609  and the p-value is 6.638e-12, which is smaller than 0.05. We can can conclude that the interaction effect of age and HDI is significant predictor.


```{r}
model10 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + age*`gdp_for_year ($)`, data = suicideFinal)

anova(model1,model10)
```
The F statistic is 27.473  and the p-value is 2.2e-16, which is smaller than 0.05. We can can conclude that the interaction effect of age and gdp for year is significant predictor.
...
```
The F statistic is 5.4293  and the p-value is 6.15e-05, which is smaller than 0.05. We can can conclude that the interaction effect of suicides_no and population is significant predictor.

```{r}
model12 <- lm(formula = `suicides/100k pop` ~ sex + age + population + HDI + `gdp_for_year ($)` + `gdp_per_capita ($)` + region + age*region, data = suicideFinal)
